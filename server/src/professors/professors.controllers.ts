...
export const createProfessor = async (req: Request, res: Response): Promise<void> => {
  try {
    const { area_academica_id, fecha_contratacion, educational_level_id, estado_empleado } = req.body
    const professor = await professorService.createProfessor({ area_academica_id, fecha_contratacion, educational_level_id, estado_empleado })
    res.json(professor)
  } catch (err: any) {
    console.error(err) // Log para ver el error
    res.status(500).send({ error: 'Server Error', details: err.message })
